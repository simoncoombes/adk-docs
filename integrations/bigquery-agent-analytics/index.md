...
The four workflow views (`v_agent_transfer`, `v_agent_state_checkpoint`, `v_event_compaction`, `v_tool_paused`) and the `pause_kind` / `function_call_id` columns on `v_tool_completed` come with the [ADK 2.0 workflow event support](#adk-2-events).
